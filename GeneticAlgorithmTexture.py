...
def goblet(filename, rgb, texturefile, saveTime):
    rgb = rgb.split(',')
    r = int(rgb[0])
    g = int(rgb[1])
    b = int(rgb[2])
    ...
    first = 0
    print(rows, cols)
    # imagegaojiaobeiArray = scan_line_seed(rows/2, cols/2, imagegaojiaobeiArray,r,g,b)
    rangev = 40
    rcolor = 240
    gcolor = 0
    bcolor = 255
    ...
    imagecaizhi = Image.open(texturefile)
    imagecaizhi = imagecaizhi.resize(images.size)
    imagecaizhiArray = np.array(imagecaizhi)
    rows, cols, dims = imagecaizhiArray.shape
    print(dims)
    for x in range(rows):
        for y in range(cols):
            if imagegaojiaobeiArray[x, y, 0] != r and imagegaojiaobeiArray[x, y, 1] != g and imagegaojiaobeiArray[
                x, y, 2] != b:
                imagecaizhiArray[x, y, 0] = 255
                imagecaizhiArray[x, y, 1] = 255
                imagecaizhiArray[x, y, 2] = 255

    imagecaizhi = Image.fromarray(imagecaizhiArray)
    # r,g,b,a = imagecaizhi.split()
    # images.paste(imagecaizhi, (0,0), mask = a)
    imgBlend = Image.blend(images, imagecaizhi, 0.5)

    imgBlend.save("C:\\apache-tomcat-7.0.53\\wtpwebapps\\art0804\\image\\gaojiaobeiTextureA_"+saveTime+".jpg")
    imgBlend.save("C:\\apache-tomcat-7.0.53\\wtpwebapps\\art0804\\image\\gaojiaobeiPartA_" + saveTime + ".jpg")
    #imgBlend.save("gaojiaobeiTexture_"+saveTime+".jpg")

if __name__ == '__main__':
    # goblet("C:/apache-tomcat-7.0.53/wtpwebapps/art0804/image/gaojiaobei 1.jpg","33,55,112","C:/apache-tomcat-7.0.53/wtpwebapps/art0804/image/M22.jpg","64384")
    goblet(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4])
