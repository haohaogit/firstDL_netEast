...
def cookerAssemble(content,timeid):
    ...
    for itemA in S:
        for itemB in K:
            for itemC in C:
                for itemD in D:
                    s1 = s1u[itemA] * s1q['A'] + s1u[itemB] * s1q['B'] + s1u[itemC] * s1q['C'] + s1u[itemD] * s1q[
                        'D'] + s1c
                    s2 = s2u[itemA] * s2q['A'] + s2u[itemB] * s2q['B'] + s2u[itemC] * s2q['C'] + s2u[itemD] * s2q[
                        'D'] + s2c
                    s3 = s3u[itemA] * s3q['A'] + s3u[itemB] * s3q['B'] + s3u[itemC] * s3q['C'] + s3u[itemD] * s3q[
                        'D'] + s3c
                    s4 = s4u[itemA] * s4q['A'] + s4u[itemB] * s4q['B'] + s4u[itemC] * s4q['C'] + s4u[itemD] * s4q[
                        'D'] + s4c
                    s5 = s5u[itemA] * s5q['A'] + s5u[itemB] * s5q['B'] + s5u[itemC] * s5q['C'] + s5u[itemD] * s5q[
                        'D'] + s5c
                    # s6 = s6u[itemA] * s6q['A'] + s6u[itemB] * s6q['B'] + s6u[itemC] * s6q['B'] + s6u[itemD] * s6q['D'] + s6c
                    value = math.sqrt((s1 - float(content[0])) * (s1 - float(content[0])) + (s2 - float(content[1])) * (
                    s2 - float(content[1])) + (s3 - float(content[2])) * (s3 - float(content[2])) + (
                                      s4 - float(content[3])) * (s4 - float(content[3])) + (s5 - float(content[4])) * (
                                      s5 - float(content[4])))
    ...
    #print(minitemS)
    boat.save("C:/apache-tomcat-7.0.53/wtpwebapps/art0804/image/cooker_"+timeid+".jpg")
    boat.save("C:/apache-tomcat-7.0.53/wtpwebapps/art0804/image/cookerColor_"+timeid+".jpg")
    boat.save("C:/apache-tomcat-7.0.53/wtpwebapps/art0804/image/cookerTexture_" + timeid + ".jpg")
    boat.save("C:/apache-tomcat-7.0.53/wtpwebapps/art0804/image/cookerPart_" + timeid + ".jpg")
...
if __name__ == '__main__':
    #print("aaaaaaaaaa")
    cookerAssemble(sys.argv[1],sys.argv[2])
